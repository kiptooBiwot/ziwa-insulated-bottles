...
<template>
  ...
  >
    ...
    <div class="flex items-center">
      ...
      <div v-if="userStore.user" class="px-2">
        <h4 class="text-sm text-[#395A9F]">
          Hello, {{ userStore.user?.name.split(' ')[0] }}
        </h4>
      </div>
      <div class="relative">
        <button
          @click="dropdownOpen = !dropdownOpen"
          class="relative z-10 block w-8 h-8 overflow-hidden rounded-full shadow transform duration-500 ease-in-out hover:scale-105 hover:shadow-xl focus:outline-none"
        >
          <Icon name="ic:baseline-account-circle" class="w-full h-full" />
