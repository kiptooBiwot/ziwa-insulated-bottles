...
<template>
  <div class="min-h-screen">
    ...
    <div class="flex flex-row gap-0 flex-wrap">
      <div
        v-for="(item, index) in orderStore.dashboardSummary"
        :key="index"
        class="mb-10 w-full max-w-full flex-shrink px-2 sm:w-1/2 lg:w-1/4"
      >
        <div class="h-full rounded-xl bg-white shadow-2xl">
          ...
          <div class="flex flex-row justify-between px-6 py-4">
            <div
              class="relative h-14 w-14 self-center rounded-full text-center text-pink-50"
              :class="`${item.color}`"
            >
              <Icon
                :name="item.icon"
                class="absolute left-1/2 top-1/2 h-8 w-8 -translate-x-1/2 -translate-y-1/2 transform"
              />
              <!-- <svg
            ...
            <h2 class="self-center text-3xl font-bold">{{ item.number }}</h2>
          </div>
          <!-- <div class="px-6 pb-6">
