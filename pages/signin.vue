...
const rules = computed(() => {
  return {
    email: {
      required: helpers.withMessage('A registered email is required to sign in', required),
      email: helpers.withMessage('The email entered is not a valid email', email)
    },
    password: {
      required: helpers.withMessage('A password is required to sign in', required)
    }
  }
})
        ...
        <div class="flex items-center h-full px-20 bg-gray-900 bg-opacity-40">
          <div>
            ...
            <p class="max-w-xl mt-3 text-gray-300">
              Administrator sign in page. Sign in to manage orders and manage inventory.
            </p>
          </div>
        </div>
      </div>
