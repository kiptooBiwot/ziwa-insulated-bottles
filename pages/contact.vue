    ...
    try {
      ...
      if (response.statusCode === 200) {
        toast.add({
          type: 'success',
          message:
            'Your message was received. We will reach out to you as soon as possible',
          timeout: 6000,
        })
...
<template>
  <div class="w-full pb-20 min-h-screen">
    <div class="w-full bg-blue-200 py-5">
      <div class="max-w-6xl mx-auto py-5">
        <h3 class="px-5 lg:px-0 text-2xl font-semibold">Contact Us</h3>
      </div>
    </div>
    <div class="grid grid-cols-1 max-w-4xl lg:max-w-full lg:grid-cols-2">
      ...
      <div class="p-5">
        <div v-if="!emailSent" class="w-full md:w-[80%] mx-auto space-y-5">
          <h5 class="text-lg">
            Fill in the form below to drop us a message or inquiry.
          </h5>
          ...
          <form action="" class="space-y-3" @submit.prevent="sendEmail">
            <div class="flex flex-col">
              ...
              <input
                ...
                class="py-2 px-4 border rounded-md w-full text-sm"
                :class="{
            ...
            <div class="flex flex-col">
              <label for="mobileNumber" class="text-xs"> Mobile Number </label>
              <input
                v-model="contactDetails.mobileNumber"
                type="text"
                name="mobileNumber"
            ...
            <div class="flex flex-col">
              ...
              <textarea
                v-model="contactDetails.contactMessage"
                name="message"
                id=""
                cols="30"
                rows="5"
              ...
              <div>
                <span
                  class="text-xs text-red-500"
                  v-if="v$.contactMessage.$error"
                  >{{ v$.contactMessage.$errors[0].$message }}</span
                >
              </div>
            </div>
            <div>
              <button
                type="submit"
                class="py-3 px-6 w-full text-white bg-green-500 uppercase text-sm rounded-md hover:bg-gree-600 hover:shadow-lg hover:text-gray-200"
              >
                Submit
              </button>
            </div>
        ...
        <div v-else class="w-full md:w-[80%] mx-auto space-y-5">
          <div class="w-full bg-green-100 px-10 py-5 rounded-md">
            <h3 class="text-green-700 font-bold text-xl pb-3">
              Your message was received
            </h3>
            <p class="text-green-500">
              Your message is well received. Thank you for reaching out to us.
